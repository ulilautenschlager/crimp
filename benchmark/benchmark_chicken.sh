#!/bin/bash
# usage ./benchmark_chicken.sh <number of replicates>

n_rep=$1

source utils.sh

echo -e "method\tsystem\tuser\telapsed\tswaps\tmax_res_kb\tShannon_ent\tGini_imp\tH\tHprime" > results_chicken.txt
...
for obj in gini_imp entropy; do
    for n_iter in 1 5 20 100; do
        ...
        for i_rep in $(seq 1 $n_rep); do
            ...
            # prepare input
            create_crimp_input input_shuffled/chicken/$i_rep > input_crimp.txt
            ...
            if grep -q finished time.txt; then
                # evaluate output
                rearrange_columns input_shuffled/chicken/$i_rep input_crimp.txt.permutations > q_matrices_ordered.txt
                scores="$(evaluate_scores q_matrices_ordered.txt)"
                echo -e "${method}\t$(head -n1 time.txt)\t$scores" >> results_chicken.txt
            else
                echo -e "${method}\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA" >> results_chicken.txt
                break
            fi
    ...
    for obj in H Hprime; do
        for n_iter in 1 5 20 100; do
            ...
            for i_rep in $(seq 1 $n_rep); do
                ...
                # prepare input
                create_clumpp_input input_shuffled/chicken/$i_rep > input_clumpp.indfile
                ...
                if grep -q finished time.txt; then
                    ...
                    rearrange_columns input_shuffled/chicken/$i_rep permutations > q_matrices_ordered.txt
                    scores="$(evaluate_scores q_matrices_ordered.txt)"
                    echo -e "${method}\t$(head -n1 time.txt)\t$scores" >> results_chicken.txt
                else
                    echo -e "${method}\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA" >> results_chicken.txt
...
for i_rep in $(seq 1 $n_rep); do
    ...
    # prepare input
    create_pophelper_input input_shuffled/chicken/$i_rep > input_pophelper.txt
    ...
    if grep -q finished time.txt; then
        # evaluate output
        rearrange_columns input_shuffled/chicken/$i_rep permutations.csv > q_matrices_ordered.txt
        scores="$(evaluate_scores q_matrices_ordered.txt)"
        echo -e "${method}\t$(head -n1 time.txt)\t$scores" >> results_chicken.txt
    else
        echo -e "${method}\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA" >> results_chicken.txt
...
for dist in G jaccard; do
    ...
    for i_rep in $(seq 1 $n_rep); do
        ...
        # prepare input
        create_pong_input input_shuffled/chicken/$i_rep > filemap_pong.txt
        
        # run pophelper
        timeout 1800s /usr/bin/time -o time.txt -f "%S\t%U\t%e\t%W\t%M" \
            pong -m filemap_pong.txt --dist_metric $dist -s 0 -o pong_output -f --disable_server -g -v \
        && echo finished >> time.txt
