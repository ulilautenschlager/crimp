#!/bin/bash
# usage ./benchmark_largeR.sh <number of replicates>

n_rep=$1

source utils.sh

echo -e "method\tsystem\tuser\telapsed\tswaps\tmax_res_kb\tShannon_ent\tGini_imp\tH\tHprime" > results_largeR.txt
...
for obj in gini_imp entropy; do
    for n_iter in 1 5 20 100; do
        ...
        for i_rep in $(seq 1 $n_rep); do
            ...
            # prepare input
            create_crimp_input input_shuffled/largeR/$i_rep > input_crimp.txt
            ...
            if grep -q finished time.txt; then
                # evaluate output
                rearrange_columns input_shuffled/largeR/$i_rep input_crimp.txt.permutations > q_matrices_ordered.txt
                scores="$(evaluate_scores q_matrices_ordered.txt)"
                echo -e "${method}\t$(head -n1 time.txt)\t$scores" >> results_largeR.txt
            else
                echo -e "${method}\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA" >> results_largeR.txt
                break
            fi
    ...
    for obj in H Hprime; do
        for n_iter in 1 5 20 100; do
            ...
            for i_rep in $(seq 1 $n_rep); do
                ...
                # prepare input
                create_clumpp_input input_shuffled/largeR/$i_rep > input_clumpp.indfile
                ...
                if grep -q finished time.txt; then
                    ...
                    rearrange_columns input_shuffled/largeR/$i_rep permutations > q_matrices_ordered.txt
                    scores="$(evaluate_scores q_matrices_ordered.txt)"
                    echo -e "${method}\t$(head -n1 time.txt)\t$scores" >> results_largeR.txt
                else
                    echo -e "${method}\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA" >> results_largeR.txt
...
for i_rep in $(seq 1 $n_rep); do
    ...
    # prepare input
    create_pophelper_input input_shuffled/largeR/$i_rep > input_pophelper.txt
    ...
    if grep -q finished time.txt; then
        # evaluate output
        rearrange_columns input_shuffled/largeR/$i_rep permutations.csv > q_matrices_ordered.txt
        scores="$(evaluate_scores q_matrices_ordered.txt)"
        echo -e "${method}\t$(head -n1 time.txt)\t$scores" >> results_largeR.txt
    else
        echo -e "${method}\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA" >> results_largeR.txt
...
for dist in G jaccard; do
    ...
    for i_rep in $(seq 1 $n_rep); do
        ...
        # prepare input
        create_pong_input input_shuffled/largeR/$i_rep > filemap_pong.txt
